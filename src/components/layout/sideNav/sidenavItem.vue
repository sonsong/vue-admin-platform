    ...
    <div>
        <template v-for="(route, i) in routes">
            ...
            <template v-if="route.children && route.children.length === 1 && route.single">
                <el-menu-item :index="route.path + '/' + route.children[0].path" :key="i">
            ...
            <template v-else>
                <el-submenu :index="'/' + route.path" :key="i">
                    <template slot="title">
                        <base-icon :iconName="route.meta.icon"/>
                        <span slot="title">{{route.meta.title}}</span> 
                    </template>
                    <el-menu-item v-for="(item, k) in route.children" :index="(route.path + '/' + item.path)" :key="k">
